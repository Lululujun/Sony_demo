...
运行时钟分为三种模式：普通访问使用真实推进的模拟时钟，并在软锁到期后自动回流；`demo=1` 使用真实推进时钟但关闭自动超时，由演示者点击“模拟超时”；`shot=1` 注入固定时间源并关闭自动超时，使倒计时文本、进度条和审计时间在每次截图时完全一致。
...
`.github/workflows/ci.yml` 已配置：依赖安装 → TypeScript 检查 → Vitest → 静态构建。Cloudflare Pages 通过 Git 集成自行拉取仓库并发布 `out/`，CI 不再执行 GitHub Pages 专用的 artifact 上传步骤。
